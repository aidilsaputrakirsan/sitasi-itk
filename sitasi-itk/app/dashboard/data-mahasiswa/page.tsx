...
import { useMahasiswas } from '@/hooks/useMahasiswas';
...
export default function DataMahasiswaPage() {
  ...
  const { data: mahasiswas, isLoading, error } = useMahasiswas();
          ...
          <div className="flex items-center gap-3">
            <Avatar className="h-9 w-9">
              {mahasiswa.profiles?.photo_url ? (
                <AvatarImage src={mahasiswa.profiles.photo_url} alt={mahasiswa.nama} />
              ) : (
                <AvatarFallback>{mahasiswa.nama.charAt(0).toUpperCase()}</AvatarFallback>
              )}
            </Avatar>
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem>
                <Link href={`/dashboard/data-mahasiswa/${mahasiswa.id}`} className="w-full">
                  Detail
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem>
                <Link href={`/dashboard/data-mahasiswa/${mahasiswa.id}/edit`} className="w-full">
                  Edit
                </Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
